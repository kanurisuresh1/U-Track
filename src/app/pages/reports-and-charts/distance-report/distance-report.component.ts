...
export class DistanceReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.headerService.updateHeaderTitle('Distance Report')
  ...
  onChanges() {
    ...
    this.distanceReportForm.get('startDate').valueChanges.subscribe(date => {
  ...
  new_track_report_web_mongo() {
    this.uTrackService.new_track_report_web_mongo(this.deviceLinkId,
      this.pipe.transform(this.distanceReportForm.value.startDate, 'yyyy-MM-dd HH:mm:ss'),
      this.pipe.transform(this.distanceReportForm.value.endDate, 'yyyy-MM-dd HH:mm:ss')).subscribe(response => {
        if (response.status) {
          ...
          if (response.data != null && response.data != undefined && response.data.length > 0) {
            ...
            this.updateUI(this.distanceReportForm.value.vehicleTimeInterval);
  ...
  downloadStoppageRawPDF() {
    ...
    doc.save('DistanceRawDataReport.pdf');
  }
