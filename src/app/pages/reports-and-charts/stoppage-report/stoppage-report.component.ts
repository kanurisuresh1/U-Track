...
export class StoppageReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.headerService.updateHeaderTitle('Stoppage Report')
  ...
  onChanges() {
    ...
    this.stoppageReportForm.get('startDate').valueChanges.subscribe(date => {
  ...
  new_track_report_web_mongo() {
    this.uTrackService.new_track_report_web_mongo(this.deviceLinkId,
      this.pipe.transform(this.stoppageReportForm.value.startDate, 'yyyy-MM-dd HH:mm:ss'),
      this.pipe.transform(this.stoppageReportForm.value.endDate, 'yyyy-MM-dd HH:mm:ss')).subscribe(response => {
        if (response.status) {
          ...
          if (response.data != null && response.data != undefined && response.data.length > 0) {
            ...
            this.updateUI(this.stoppageReportForm.value.vehicleTimeInterval);
  ...
  downloadStoppageRawPDF() {
    ...
    doc.save('StoppageRawDataReport.pdf');
  }
